...
import { Moon, Sun, Menu, X, Monitor, Palette, Type } from "lucide-react";
import { useTheme } from "next-themes";

// Note: Ensure @/app/layout exports firaCode or use a hardcoded font-family if it errors.
export function Navbar() {
  const { theme, setTheme } = useTheme();
  const { language, setLanguage, colorTheme, setColorTheme, fontTheme, setFontTheme, t } = useAppContext();
      ...
      <nav className="w-full fixed top-0 z-50 bg-background/80 backdrop-blur-md border-b border-border">
        <div className="max-w-[1400px] mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-10 font-sans font-medium text-sm">
            ...
            <a href="#projects" className="hover:text-primary transition-colors">{t.nav.experience}</a>
          ...
          <div className="hidden md:flex items-center gap-6 font-mono text-xs">
            <div className="flex items-center gap-3 relative">
              ...
              <div className="relative">
                <button onClick={() => setIsFontMenuOpen(!isFontMenuOpen)} className="p-1 hover:text-primary transition-colors">
                  <Type className="w-4 h-4" />
                </button>
                {isFontMenuOpen && (
                  <div className="absolute top-full right-0 mt-4 w-48 bg-card border border-border rounded-xl shadow-2xl z-50 flex flex-col p-2 text-sm font-sans" onMouseLeave={() => setIsFontMenuOpen(false)}>
                    <button onClick={() => { setFontTheme('sans'); setIsFontMenuOpen(false); }} className={`px-3 py-2 text-left hover:bg-primary/10 rounded-lg transition-colors ${fontTheme === 'sans' ? 'text-primary font-bold bg-primary/5' : ''}`}>Font Sans</button>
                    <button onClick={() => { setFontTheme('jetbrains'); setIsFontMenuOpen(false); }} className={`px-3 py-2 text-left hover:bg-primary/10 rounded-lg transition-colors ${fontTheme === 'jetbrains' ? 'text-primary font-bold bg-primary/5' : ''}`}>JetBrains Mono</button>
                  </div>
                )}
              </div>
              <button
            ...
            <div className="flex flex-col gap-2 pt-4 border-t border-border mt-2">
              ...
              <div className="flex flex-col gap-2 mt-4 text-xs text-muted-foreground">
                <div className="flex gap-2 flex-wrap">
                  <button onClick={() => setFontTheme('jetbrains')} className={`px-3 py-1.5 rounded-md border ${fontTheme === 'jetbrains' ? 'border-primary text-primary' : 'border-border'}`}>JetBrains Mono</button>
                  <button onClick={() => setFontTheme('sans')} className={`px-3 py-1.5 rounded-md border ${fontTheme === 'sans' ? 'border-primary text-primary' : 'border-border'}`}>Font Sans</button>
                </div>
              </div>
            </div>
          </div>
