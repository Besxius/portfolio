...
const styles = `
.split-flap-text{font-family:'SFMono-Regular','Roboto Mono','Cascadia Code','Liberation Mono',Menlo,monospace;font-size:var(--split-flap-font-size,52px);font-weight:760;line-height:1;letter-spacing:.035em;font-variant-numeric:tabular-nums;display:inline-flex;justify-content:center;align-items:center;gap:var(--split-flap-gap,6px)}
...
// Căn giữa văn bản bằng cách thêm khoảng trắng đều vào cả 2 bên
const normalizePhrase = (phrase: string, width: number) => {
    const safe = String(phrase ?? '').slice(0, width);
    const padTotal = Math.max(0, width - safe.length);
    const padStart = Math.floor(padTotal / 2);
    const padEnd = Math.ceil(padTotal / 2);
    return ' '.repeat(padStart) + safe + ' '.repeat(padEnd);
};
...
const SplitFlapText = ({
    ...
    return (
        <>
            <style>{styles}</style>
            <div
                className={`split-flap-text whitespace-pre select-none ${className}`.trim()}
